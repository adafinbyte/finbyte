...
const ForumsPostView: FC <custom_props> = ({
  ...
  return (
    <div className="flex flex-col w-full gap-2">
      {create_action.state && tab_action.state !== 1 && !(tab_action.state === 4 && !is_team) && (
        <PostCreation
      ...
      <div className="text-neutral-400 flex gap-1 items-center">
        <button title="Sort By: Timestamp" onClick={() => set_sorting_by('timestamp')} className="p-2 inline-flex gap-2 items-center hover:bg-neutral-800 rounded-lg">
          <Clock size={16} className={`${sorting_by === 'timestamp' ? 'text-blue-400' : ''}`}/>
        </button>

        <button title="Sort By: Likes" onClick={() => set_sorting_by('likes')} className="p-2 inline-flex gap-2 items-center hover:bg-neutral-800 rounded-lg">
          <Heart size={16} className={`${sorting_by === 'likes' ? 'text-blue-400' : ''}`}/>
        </button>

        <button title="Sort By: Comments" onClick={() => set_sorting_by('comments')} className="p-2 inline-flex gap-2 items-center hover:bg-neutral-800 rounded-lg">
        ...
        {sorted_posts ?
          <div className="flex flex-col w-full gap-2">
            {sorted_posts.length > 0 ?
              ...
              :
              <div className="p-2 text-neutral-400 text-center">
                no posts to show
              </div>
            }
          </div>
