...
const TokensBlock: FC = () => {
  ...
  const filtered_tokens = curated_tokens.filter((item) =>
    item.name.toLowerCase().includes(search_query.toLowerCase()) ||
    item.category.toLowerCase().includes(search_query.toLowerCase()) ||
    item.token_details.ticker.toLowerCase().includes(search_query.toLowerCase())
  );
          ...
          >
            <div className="grid gap-2">
              ...
              <Input
                ...
                placeholder="Search token by name, ticker or category..."
              />
            </div>
          </motion.div>
                    ...
                    >
                      ...
                      <CardContent className="relative space-y-2">
                        <h3 className="font-medium text-gray-900 dark:text-gray-100 tracking-tight text-[15px]">
                          {token.name}
                          <span className="ml-2 text-xs text-gray-500 dark:text-gray-400 font-normal"><span className="pr-0.5 text-green-400">$</span>{token.token_details.ticker}</span>
                        </h3>

                        <ScrollArea className="w-full">
                          <p className="text-sm h-12 text-gray-600 dark:text-gray-300 leading-snug font-[425]">{token.description}</p>
                        </ScrollArea>
                      </CardContent>

                      <div className="flex items-center justify-end w-full p-2 px-4">
                        <span className="inline-flex gap-1 items-center text-xs text-gray-500 dark:text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">
                          Explore
