...
const FormatAddress: FC <custom_props> = ({
  ...
  return (
    <div className={`inline-flex items-center ${themed['200'].text}`} {...props}>
      {address.startsWith('addr1') ?
        <span className="inline-flex gap-1 items-center">
          <span className={`${themed['500'].text}`}>{address.substring(0, 5) + "..."}</span>
        ...
        <span className="inline-flex gap-1 text-sm items-center">
          ...
          <span className={`w-20 lg:w-full truncate ${themed['300'].text}`}>{address}</span>
        </span>
      }
    </div>
  )
