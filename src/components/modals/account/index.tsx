...
import { AssetExtended, BlockfrostProvider, MeshWallet } from "@meshsdk/core"
...
const AccountModal: FC <custom_props> = ({
  open, onOpenChange, address, platform_user_details
}) => {
  const { getBalance, wallet } = useConnectWallet();
  const provider = new BlockfrostProvider(blockfrost_key);

  const meshsdk = new MeshWallet({
    networkId: 1,
    fetcher: provider,
    key: {
      type: 'address',
  ...
  const [is_anon_user, set_is_anon_user] = useState(true);
  const [ada_balance, set_ada_balance] = useState(0);
  ...
  type view = 'Overview' | 'Create Account' | 'Settings' | 'Details';
  ...
  const create_finbyte_account = async () => {
  }
  ...
  useEffect(() => {
    getBalance(set_ada_balance);
    find_users_adahandles();
    set_is_anon_user(platform_user_details.type === 'anon');
  }, []);
  ...
  const basic_views: Partial<Record<view, ReactNode>> = {
      ...
      <div className="max-h-72">
        <div className="flex w-full justify-between items-center gap-2">
          ...
          <Badge variant={'outline'}>
            {is_anon_user ? 'Anonymous' : 'Registered'}
          </Badge>
        </div>
        ...
        <Card className="mt-2 relative p-4 border-transparent dark:bg-white/5 bg-black/5">
          ...
          <div className="grid grid-cols-2 gap-2" style={{placeItems: 'start'}}>
            <div className="text-center w-full">
              <Label>Wallet Balance</Label>
              <h1 className="text-lg">
                <sub className="text-blue-500 dark:text-blue-400">₳ </sub>{ada_balance}
              </h1>
            </div>

            <div className="text-center w-full">
              <Label>Finbyte Kudos</Label>
              <h1 className="text-lg">
                <sub className="text-blue-500 dark:text-blue-400">Ƒ </sub>{platform_user_details.finbyte_kudos}
              </h1>
            </div>
          </div>
        </Card>
          ...
          <div className="flex flex-col gap-1 w-full mt-2">
            <div className="flex w-full items-center justify-between gap-2">
              ...
              <Badge variant='secondary'>
                {is_anon_user ?
                  ...
                  : platform_user_details.ada_handle ?
                  <span className="text-black dark:text-white/60 text-sm inline-flex items-center">
                    ...
                    {platform_user_details.ada_handle?.slice(1)}
                  </span>
                  : 'Not Set'
                }
              </Badge>
            ...
            <div className="flex w-full items-center justify-between gap-2">
              ...
              <Badge variant='secondary'>
                {platform_user_details.total_posts ?? 0}
              </Badge>
            </div>
            <div className="flex w-full items-center justify-between gap-2">
              <Label>First Interacted</Label>
              <Badge variant='secondary'>
                {platform_user_details.first_timestamp ?
                  format_unix(platform_user_details.first_timestamp).date :
                  'Never Interacted'
                }
              </Badge>
            </div>
    ...
    'Details': (
      <div className="max-h-72">
        <div className="flex w-full justify-between items-center gap-2">
          <Label className="text-center pb-2">Account Details</Label>
        </div>
        ...
        <div className="flex flex-col gap-1 w-full mt-2">
          <div className="flex w-full items-center justify-between gap-2">
            <Label>Status</Label>
            <Badge variant='secondary'>
              {is_anon_user ? 'Anonymous User' : 'Finbyte User'}
            </Badge>
          </div>
          <div className="flex w-full items-center justify-between gap-2">
            <Label>Username</Label>
          ...
          <div className="flex w-full items-center justify-between gap-2">
            <Label>First Interacted</Label>
            <Badge variant='secondary'>
              {platform_user_details.first_timestamp !== 0 ?
                format_unix(platform_user_details.first_timestamp).date :
                'Never Interacted'
              }
            </Badge>
  ...
  const views = platform_user_details.account_data?.address === null ? { ...basic_views, ...non_finbyte_user_views } : {...basic_views, ...finbyte_user_views};
