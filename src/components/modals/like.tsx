...
import { Copy, HeartCrack, HeartHandshake, Share, X } from 'lucide-react';
...
interface custom_props {
  ...
  is_liker: boolean;
  on_like_unlike: () => Promise<void> | undefined;
}

const LikeModal: FC <custom_props> = ({
  is_modal_open, close_modal, is_liker, on_like_unlike
}) => {
  useEffect(() => {
    if (is_modal_open) {
    ...
    >
      <div className="bg-neutral-900 border border-neutral-800 shadow-md shadow-neutral-950/40 rounded-lg w-9/12 sm:w-7/12 md:w-5/12 lg:w-3/12 scale-95 transition-transform duration-300 ease-out">
        ...
        <div className="border-b border-neutral-800 p-4 flex justify-between items-center items-start">
          <div className="flex flex-col">
            <div className='inline-flex items-center gap-2'>
              {is_liker ? <HeartCrack size={18} className='text-rose-400'/> : <HeartHandshake size={18} className='text-rose-400'/>}
              <h1 className="text-lg text-neutral-300 font-semibold">{is_liker ? 'Unlike' : 'Like'} Post</h1>
            </div>
            <p className="text-sm text-neutral-500">You can like and unlike this post here.</p>
          </div>
        ...
        <div className="p-4 text-neutral-300 text-center w-full max-w-content text-left text-base flex flex-col justify-start gap-1 h-40 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-neutral-700 [&::-webkit-scrollbar-thumb]:bg-neutral-500">
          <p className='text-sm'>
            You are about to {is_liker ? 'unlike' : 'like'} this post, are you sure?
          </p>

          <div className='flex gap-2 flex-wrap items-center justify-center my-auto text-sm'>
            ...
            <button onClick={handle_action} className='p-2 duration-300 hover:bg-neutral-800 rounded-lg inline-flex gap-2 items-center'>
              Confirm
              {is_liker ? <HeartCrack size={16}/> : <HeartHandshake size={16}/>}
            </button>
          </div>
        </div>
      </div>
