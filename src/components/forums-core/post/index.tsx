...
import { ArrowRight, Heart, HeartCrack, HeartHandshake, MessageCircle } from "lucide-react";
...
const ForumPost: FC <custom_props> = ({
  post_type, preview, forum_post, forum_comment, community_post
}) => {
  ...
  return (
    <>
      <div className={`${preview ? 'hover:border-blue-400' : ''} duration-300 border border-neutral-700 rounded-lg bg-neutral-900 w-full text-neutral-400`}>
        ...
        <div className="border-t border-neutral-700 p-2">
          <div className="flex gap-2 items-center w-full">
            <button onClick={() => set_view_likers_modal_open(true)} className="inline-flex items-center gap-2 hover:bg-neutral-800 rounded-lg p-2">
              <HeartHandshake size={16} className={`${post_likers.includes(use_wallet.address) ? 'text-rose-400' : ''}`}/>
            ...
            {post_type === 'community_post' && (
              <button onClick={() => set_like_modal_open(true)} className="text-sm inline-flex items-center gap-2 hover:bg-neutral-800 rounded-lg p-2">
                {community_post?.post.post_likers?.includes(use_wallet.address) ? 'Unlike Comment' : 'Like Comment'}
                {community_post?.post.post_likers?.includes(use_wallet.address) ?
                  <HeartCrack size={16} />
                  :
                  <Heart size={16} />
                }
            ...
            {post_type === 'forum_comment' && (
              <button onClick={() => set_like_modal_open(true)} className="text-sm inline-flex items-center gap-2 hover:bg-neutral-800 rounded-lg p-2">
                {forum_comment?.post.post_likers?.includes(use_wallet.address) ? 'Unlike Comment' : 'Like Comment'}
                {forum_comment?.post.post_likers?.includes(use_wallet.address) ?
                  <HeartCrack size={16} />
                  :
                  <Heart size={16} />
                }
            ...
            {post_type === 'forum_post' && (
              <button onClick={() => set_view_repliers_modal_open(true)} className="inline-flex items-center gap-2 hover:bg-neutral-800 rounded-lg p-2">
            ...
            {post_type === 'forum_post' && preview && (
              <Link href={'forums/' + forum_post?.post.post.id} className="ml-auto group">
                <button className="italic text-sm group-hover:bg-neutral-800 p-2 px-4 rounded-lg inline-flex gap-2 items-center">
                  View Post
